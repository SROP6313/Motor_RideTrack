...
def process_imu_data(input_folder, output_file, app_time_error):
    # Load the CSV files
    accelerometer_df = pd.read_csv(os.path.join(input_folder, 'Accelerometer.csv'))
    gyroscope_df = pd.read_csv(os.path.join(input_folder, 'Gyroscope.csv'))
    
    # Read the metadata time file
    metadata_time_df = pd.read_csv(os.path.join(input_folder, 'meta', 'time.csv'))
    ...
    Reverse_Axis_Data = pd.concat([accelerometer_df, gyroscope_df.drop(columns=['Time (s)'])], axis=1)
...
# Usage example
input_folder = './20240708_data/20240708_Liyi'
output_file = './20240708_data/20240708_Liyi_sliding_window.csv'
app_time_error = 0

process_imu_data(input_folder, output_file, app_time_error)
